...
describe('AuthServices', () => {
  describe('isAllowedToRegister', () => {
    it.each([
      ...
      ['return true with multiple org in env', 'testorg', ['testorg', 'org2'], true],
      ['return true with single org not in env', 'testorg,org2', ['testorg'], true],
    ])('will %s', (_, allowedOrgs, orgs, result) => {
      process.env.ALLOWED_ORGS = allowedOrgs
      expect(
        AuthServices.isAllowedToRegister(orgs)
      ).toEqual(result)
    })
  })
  ...
  describe('githubStrategyCallback', () => {
    ...
    it('will return an error if user is not allowed to register', async () => {
      const mockIsAllowedToRegister = jest.spyOn(AuthServices, 'isAllowedToRegister').mockReturnValueOnce(false)
      const done = jest.fn()
      ...
      expect(mockIsAllowedToRegister).toHaveBeenCalledWith(profileResponse.organizations)
    ...
    it('will create a user when one does not exist', async () => {
      ...
      const user = await prisma.user.findFirst({ where: { email: profileResponse.email } })
    ...
    it('will update a user when one does exist', async () => {
      ...
      const user = await prisma.user.findFirst({ where: { id: createdUser.id } })
      expect(user).toBeDefined()
