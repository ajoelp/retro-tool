...
describe('CardsController', () => {
  ...
  describe('list', () => {
    ...
    it('will list all the cards for a column', async () => {
      const otherColumn = await prisma.column.create(({ data: { title: 'other column', boardId: board.id }}))
      const card1 = {ownerId: 'ownerId', columnId: column.id, content: 'random-content'}
      const card2 = {ownerId: 'ownerId', columnId: column.id, content: 'random-content2'}
      const card3 = {ownerId: 'ownerId', columnId: otherColumn.id, content: 'random-content'}
      ...
      expect(response.body.cards).toEqual(expect.arrayContaining([
        expect.objectContaining(card1),
        expect.objectContaining(card2)
      ]))
      expect(response.body.cards).not.toEqual(expect.arrayContaining([
        expect.objectContaining(card3),
      ]))
