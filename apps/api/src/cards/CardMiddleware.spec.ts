  ...
  describe('canEditCard', () => {
    ...
    beforeAll(async () => {
      user = await prisma.user.create({
        data: { email: 'test@example.com', 'githubNickname': 'testUser', avatar: '' }
      })
      board = await prisma.board.create({ data: { title: 'board-title', ownerId: user.id } })
      column = await prisma.column.create({ data: { title: 'column-title', boardId: board.id, order: 0 } })
      card = await prisma.card.create({ data: { content: '', columnId: column.id, ownerId: user.id, order: 2, } })
    })
    ...
    it('will throw an error if current user is not the card owner', async () => {
      const differentOwner = await prisma.user.create({
        data: { email: 'test2@example.com', 'githubNickname': 'testUser2', avatar: '' }
      })
      const cardWithWrongOwner = await prisma.card.create({ data: { content: '', columnId: column.id, ownerId: differentOwner.id, order: 2, } })
      const request = {
