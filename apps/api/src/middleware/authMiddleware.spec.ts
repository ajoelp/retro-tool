...
describe('authMiddleware', () => {
  ...
  beforeAll(async () => {
    user = await prisma.user.create({ data: { email: 'test@example.com', githubNickname: 'test', avatar: '' } })
  })
  ...
  it('will throw an error when there is no auth header', () => {
    ...
    expect(() => authenticatedMiddleware(request, response, next)).rejects.toEqual(expect.any(AuthenticationError))
  })
