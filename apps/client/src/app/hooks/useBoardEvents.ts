    ...
    (event: EventType) => {
      ...
      switch (event.type) {
        ...
        case CARD_UPDATED_EVENT_NAME:
          ...
          const columnIds = (queryClient.getQueryData<Column[]>(['columns', boardId]) ?? []).map((column) => column.id);

          // Find the previous column id
          const cardPreviousColumnId = columnIds.find((id: string) => {
            return (queryClient.getQueryData<Card[]>(['cards', id]) ?? []).find((card) => card.id === event.payload.id);
          });

          // If the id's are different remove it from the old one
          if (cardPreviousColumnId !== event.payload.columnId) {
            queryClient.setQueryData<Card[]>(['cards', cardPreviousColumnId], (oldData = []) => {
              const index = oldData.findIndex((card) => card.id === event.payload.id);
        ...
        case BOARD_USERS_EVENT_NAME:
          queryClient.setQueryData(['activeUsers', { boardId }], () => event.payload);
          return;
        default:
          return;
      }
